<template>
    ...
    <div class="charts-container">
      ...
      <div class="left-section">
        ...
        <div class="controls">
          ...
          <!-- 현재 연도를 표시하는 버튼 -->
          <button class="btn btn-secondary btn-xs" @click="changeMonth(0)">{{ formattedYear }} 년 </button>
          <!-- 다음 월 버튼 -->
          <button class="btn btn-primary btn-xs" @click="changeMonth(1)">&gt;</button>
        </div>
        <!-- 파이 차트를 위한 캔버스 -->
        ...
        <div class="profile-section">
          <div class="profile-image-wrapper">
            ...
            <label for="profile-upload" class="upload-icon" @dblclick="triggerUpload">
              <input id="profile-upload" type="file" @change="onImageChange" accept="image/*" class="profile-upload"/>
      ...
      <div class="right-section">
        ...
        <div class="form-check mt-3">
          <input class="form-check-input" type="checkbox" v-model="showSummary" id="showSummaryCheck">
          ...
          <table class="summary-table">
            ...
            <tbody>
              <tr v-for="(summary, index) in summaries" :key="index">
                <td>{{ summary.day }} 일</td>
                <td :style="{ color: summary.income !== 0 ? 'blue' : 'inherit' }">{{ formatCurrencyWithSign(summary.income, true) }}</td>
                <td :style="{ color: summary.expense !== 0 ? 'red' : 'inherit' }">{{ formatCurrencyWithSign(summary.expense, false) }}</td>
                <td>{{ summary.memo }}</td>
              </tr>
            </tbody>
            <tfoot>
              <tr>
                <td>총합</td>
                <td :style="{ color: totalIncome !== 0 ? 'blue' : 'inherit' }">{{ formatCurrencyWithSign(totalIncome, true) }}</td>
                <td :style="{ color: totalExpense !== 0 ? 'red' : 'inherit' }">{{ formatCurrencyWithSign(totalExpense, false) }}</td>
                <td></td>
              </tr>
              <tr>
                <td colspan="4">총합 합계: {{ formatCurrencyWithSign(netTotal, netTotal >= 0) }}</td>
              </tr>
            </tfoot>
          </table>
        </div>
  ...
  const profile = ref({
    ...
    imageUrl: "https://via.placeholder.com/150" // 초기 프로필 이미지 URL
  });
  ...
  const fetchData = async (date) => {
    ...
    totalSum.value = summaries.value.reduce((acc, summary) => acc + summary.total, 0);
    totalIncome.value = summaries.value.reduce((acc, summary) => acc + summary.income, 0);
    totalExpense.value = summaries.value.reduce((acc, summary) => acc + summary.expense, 0);
    ...
    monthlySummaries.value = Object.values(monthData).sort((a, b) => a.month - b.month);
  ...
  const updateCharts = async (date) => {
    const { incomeData, expenseData, labels, backgroundColors, borderColors } = await fetchData(date);
    // 기존 파이 차트 인스턴스 제거
    if (pieChartInstance) {
    ...
    pieChartInstance = createChart(pieCtx, "pie", labels, [incomeData, expenseData], backgroundColors, borderColors, true, "수입과 지출");
    
    // 새로운 월별 총합 바 차트 생성
    const barCtx = document.getElementById("barChart").getContext("2d");
    const monthlyLabels = monthlySummaries.value.map(item => `${item.month}월`);
    barChartInstance = createMonthlyChart(barCtx, monthlyLabels, monthlySummaries.value);
  };
